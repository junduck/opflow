...
protected:
  ...
  void verify_depth_correctness(const T &root, const std::unordered_map<T, size_t> &depth_map,
    ...
    for (const auto &[node, depth] : depth_map) {
      for (const auto &dependent : sorter.dependents(node)) {
...
TEST_F(TopoBfsTest, GraphStructureIntegrity) {
  ...
  auto original_dependents_1 = int_sorter.dependents(1);
  auto original_dependents_2 = int_sorter.dependents(2);
  auto original_dependents_3 = int_sorter.dependents(3);

  auto [colour_map, depth_map] = int_sorter.bfs(1, topological_sorter<int>::noop_visitor);

  // Verify graph structure is unchanged
  EXPECT_EQ(int_sorter.dependents(1), original_dependents_1);
  EXPECT_EQ(int_sorter.dependents(2), original_dependents_2);
  EXPECT_EQ(int_sorter.dependents(3), original_dependents_3);
}
